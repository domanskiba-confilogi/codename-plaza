...
import { NavItem } from "./component_navbar.js";
...
function mustQuerySelector<T extends Element>(rootElement: Element, selector: string): T {
	const input = rootElement.querySelector<T>(selector);
	if (input === null) {
...
const LOGGED_IN_NAVBAR_ITEMS: NavItem[] = [
...
const LOGGED_OUT_NAVBAR_ITEMS: NavItem[] = [
...
function reportCriticalError(error: any) {
	let body: string = "";

	if (error instanceof Error) {
		body = `<pre class="overflow-x-auto">${error}\r\n${error.stack}</pre>`
	} else {
		body = `<pre class="overflow-x-auto">${String(error)}</pre>`
	}
...
function mountOnTableEndSeen(id: string, callback: () => Promise<void>) {
	const sentinel = mustQuerySelector<HTMLElement>(document.body, id);
