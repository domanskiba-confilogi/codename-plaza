function mountTextField(selector, options = {}) {
	const root = document.querySelector(selector);
	if (!root) throw new Error(`mountTextField: no element found for selector "${selector}"`);

	const allowedTypes = new Set(["text", "email", "password"]);
	const id = `input-${Math.floor(Math.random() * 1e8)}`;
	const errorId = `${id}-error`;

	const cfg = {
		type: allowedTypes.has((options.type || "text").toLowerCase()) ? options.type.toLowerCase() : "text",
...
`;
	...
	return {
		...
		getType: () => state.type,
		setType: (nextType) => {
			const t = String(nextType || "").toLowerCase();
